...
class CleoCompilerTest {
  ...
  @Test
  fun `compiles OOP methods like Player Defined and Actor Driving`() {
    val d = "$"
    val script = """
      {${d}CLEO .csa}
      :START
      wait 500
      Player.Defined(0)
      Actor.Driving(${d}PLAYER_ACTOR)
      end_thread
    """.trimIndent()

    val result = CleoCompiler.compile(script)
    assertTrue("Debe compilar métodos OOP: $result", result is CompilationResult.Success)
    val success = result as CompilationResult.Success
    assertTrue(success.bytecode.isNotEmpty())
  }

  @Test
  fun `compiles structured if then else end blocks`() {
    val d = "$"
    val script = """
      {${d}CLEO .csa}
      thread 'CARMONEY'
      ...
      wait 100
      if
        00DF: actor ${d}PLAYER_ACTOR driving
      then
        0109: player ${d}PLAYER_CHAR money += 5000
      else
        wait 500
      end
      jump @MAIN_LOOP
    ...
    val result = CleoCompiler.compile(script)
    assertTrue("Debe compilar bloque if-then-else-end: $result", result is CompilationResult.Success)
    val success = result as CompilationResult.Success
    assertTrue(success.bytecode.isNotEmpty())
  }

  @Test
  fun `compiles template keywords defined stat pressed_key in_any_car stone`() {
    val d = "$"
    val script = """
      {${d}CLEO .csa}
      thread 'TESTKW'
      ...
      0652: 1@ = player ${d}PLAYER_CHAR stat 121
    ...
    val result = CleoCompiler.compile(script)
    assertTrue("Debe compilar opcodes con palabras de plantilla: $result", result is CompilationResult.Success)
    val success = result as CompilationResult.Success
    assertTrue(success.bytecode.isNotEmpty())
  }

  @Test
  fun `compiles long script with hundreds of lines seamlessly`() {
    val d = "$"
    val sb = StringBuilder()
    sb.appendLine("{${d}CLEO .csa}")
    sb.appendLine("thread 'BIGSCRIPT'")
    sb.appendLine(":START")
    sb.appendLine("wait 10")
    for (i in 0 until 50) {
      sb.appendLine("if")
      sb.appendLine("  Player.Defined(0)")
      sb.appendLine("then")
      sb.appendLine("  0109: player ${d}PLAYER_CHAR money += $i")
      sb.appendLine("  00E1: player 0 pressed_key 17")
      sb.appendLine("else")
      sb.appendLine("  wait 50")
      sb.appendLine("end")
    }
    sb.appendLine("jump @START")

    val result = CleoCompiler.compile(sb.toString())
    assertTrue("Debe compilar script masivo sin errores: $result", result is CompilationResult.Success)
    val success = result as CompilationResult.Success
    assertTrue(success.opcodesCompiled > 100)
  }
}
